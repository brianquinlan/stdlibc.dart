...
mixin BionicStatMixin on PlatformLibC {
  @override
  Stat? stat(String file) {
    return ffi.using((arena) {
      final buf = arena<ffi.stat_t>();
      final res = bionic.stat(file.toCString(arena), buf);
  ...
  Stat? lstat(String file) {
    return ffi.using((arena) {
      final buf = arena<ffi.stat_t>();
      final res = bionic.lstat(file.toCString(arena), buf);
      return res == 0 ? buf.toStat() : null;
    });
  }
}
...
extension _BionicTimespec on ffi.timespec_t {
  DateTime toDateTime() {
